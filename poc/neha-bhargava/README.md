...
The shared starter stays in `poc\neha-bhargava`; any teammate can contribute here.
Personal POC folders remain available for experiments.
...
```powershell
Set-Location C:\Git\agent-airlock\poc\neha-bhargava
npm ci
npm run setup
npm test
```
...
agency copilot --plugin "local:C:\Git\agent-airlock\poc\neha-bhargava"
```

Or load it directly with Copilot CLI:

```powershell
copilot --plugin-dir "C:\Git\agent-airlock\poc\neha-bhargava"
```

To keep it available across Agency sessions:

```powershell
agency plugin install "local:C:\Git\agent-airlock\poc\neha-bhargava" --engine copilot
```
...
```text
poc\neha-bhargava\
  plugin.json, agency.json, .mcp.json   Host declarations; load path is unchanged
