...
// Deliberately excludes prompt text and tool arguments. The hook boundary needs
// only the decision and policy identity, minimizing sensitive data at rest.
...
function statePath(sessionId, root = dataRoot) {
  // Hashing avoids exposing a host session identifier in the file name.
...
export async function saveMissionState(state, root = dataRoot) {
  ...
  try {
    // Replace via rename so preToolUse observes either the old complete state
    // or the new complete state, never a partially written JSON document.
...
export async function loadMissionState(sessionId, root = dataRoot) {
  try {
    ...
    // Missing, corrupt, or schema-invalid state is represented uniformly.
    // decidePreToolUse treats null as a denial.
    return null;
  }
}
